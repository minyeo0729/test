...
const Selfaware = () => {
  ...
  const [cloneWidth, setCloneWidth] = useState(180);
  ...
  useEffect(() => {
    ...
    const centerX = newWrapWidth / 2 - cloneWidth / 2;
    const centerY = newWrapHeight / 2 - cloneWidth / 2;
  ...
  }, [cloneWidth, newWrapHeight, newWrapWidth, randomlyPosition]);

  useEffect(() => {
    spread();
  }, [circlePosition, boxPositions, spread]);

  const [viewBox, setViewBox] = useState('0 0 180 180');
  const [svgWidth, setSvgWidth] = useState('151');
  useEffect(() => {
    const mediaQuery = window.matchMedia('(max-width: 1280px)');

    const updateViewBox = (matches) => {
      if (matches) {
        setSvgWidth('120');
        setViewBox('0 0 150 150');
      } else {
        setSvgWidth('120');
    ...
    // Initial update based on the current media query state
    updateViewBox(mediaQuery.matches);

    const handleChange = (event) => {
      updateViewBox(event.matches);
    };
